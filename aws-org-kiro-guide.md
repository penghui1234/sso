...
> ⚠️ **在做任何 Org / root / Identity Center 操作之前，先把 B 的 Kiro 用户和实例信息完整导出留底。** 一旦后续步骤导致用户/分配丢失，可据此快速恢复或核对。

用 **账号 B 的凭证** 执行：

```bash
# 0) 建个备份目录
mkdir -p ~/kiro-backup-$(date +%Y%m%d) && cd ~/kiro-backup-$(date +%Y%m%d)

# 1) 当前身份与实例清单（记录哪个实例属于 B）
aws sts get-caller-identity > caller-identity.json
aws sso-admin list-instances > sso-instances.json

# 2) 从上一步找到 OwnerAccountId = B 账号ID 的那个 IdentityStoreId，填到下面 <B-ISID>
B_ISID=<B-IdentityStoreId>   # 形如 d-xxxxxxxxxx

# 3) 导出该实例下的全部 Kiro 用户、组、组成员
aws identitystore list-users        --identity-store-id "$B_ISID" > kiro-users.json
aws identitystore list-groups       --identity-store-id "$B_ISID" > kiro-groups.json
# 对每个 group 再导出成员（可选，有组时执行）
# aws identitystore list-group-memberships --identity-store-id "$B_ISID" --group-id <GROUP_ID> > group-members-<GROUP_ID>.json

# 4)（可选）导出 Kiro/Q 订阅分配，确认谁挂着 Pro
aws user-subscriptions list-claims 2>/dev/null > kiro-subscriptions.json || true
```

**✅ 验证点**：`kiro-users.json` 里能看到全部 Kiro 用户（含 UserName、UserId、Email），文件已妥善保存。

> 💡 这份备份在出现"用户显示 0 个"等异常时（见排障 Q8）能帮你判断到底是真丢了还是只是过渡态。
...
> ⚠️ **真正影响 Identity Center 的是这一步，不是"加入 A"。** 删除 B 旧 Org 时，B 的 Identity Center 实例可能被**重建**（实例创建时间会变成当天），Kiro 用户随后才被重新置备进新实例。请务必**先完成阶段零的备份**，删除后再用阶段一末尾的命令核对用户是否齐全。
...
### 步骤 1.5 — 确认 Identity Center 实例归属 + 用户齐全（CLI 客观核对）

加入 Org 后，在 B 里执行 `aws sso-admin list-instances` 通常会看到**多个实例**（B 自己的账号实例 + 能看到的 A 组织实例），属正常。靠 `OwnerAccountId` 区分：

```bash
aws sso-admin list-instances
```

- `OwnerAccountId` = **B 的账号ID** 的那条 → **B 的账号实例**（Kiro 挂在这上面，要保护的就是它）
- `OwnerAccountId` = 管理账号 A 的那条 → A 的组织实例，与 Kiro 无关，不用动

再核对 Kiro 用户是否都在 B 的实例里：

```bash
aws identitystore list-users --identity-store-id <B 的 IdentityStoreId>
```

**✅ 验证点**：返回的用户列表与阶段零导出的 `kiro-users.json` 一致，且每个用户 `UserStatus` 为 `ENABLED`。

> 💡 **一个组织只能有一个组织实例（归 A）**，所以 B 加入 A 之后，在 B 里看到的任何"可管理 Kiro 的实例"都只能是账号实例。
...
> ⚠️ **前置条件：先确认 Kiro 已稳定。** 回收/移除 root 属不可逆操作。执行前请确认 B 的 Identity Center 实例、Kiro 用户、Pro 订阅**已稳定一段时间**（多查几次 `list-users` 不再出现"0 个用户"的过渡态，见排障 Q8），再动 root，避免在不稳定窗口失去 root 兜底能力。
...
> 🔁 **可恢复**：方案 B 是可逆的。需要时由 A 在"集中式根访问管理"中**重新开放 B 的 root 恢复** → 走标准"忘记密码"流程、经 **B 的 root 邮箱**重置密码即可。前提是你同时掌控 **管理账号 A** 和 **B 的 root 邮箱**。把 B 移出 Organization 前也必须先恢复 root。
...
| # | 检查项 | 验证方法 | 状态 |
| --- | --- | --- | --- |
| 0 | 操作前已备份 Kiro 用户/实例 | `kiro-users.json` 等已保存 | ☐ |
...
**Q8：验证时 B 的 Kiro 用户显示 0 个，过会儿又正常了？** 多为**过渡态**，不是真丢：
- 原因一：删旧 Org 后 B 的实例被**重建**，存在**置备/最终一致性窗口**（实例已在但用户还没灌进去），等几分钟重查即可；
- 原因二：你是从 **A 的视角 / SSO** 看的——Kiro 用户挂在 B 的账号实例上，A 的组织实例里本来就看不到；
- 处理：直接在 B 内查 B 的账号实例（`identitystore list-users`），稍等重试，对照阶段零的 `kiro-users.json`，确认稳定后再继续；**别在闪现 0 用户时就动 root**。

**Q9：方案 B 移除 root 后还能恢复吗？** 能。由 A 在"集中式根访问管理"重新开放恢复 → 经 B 的 root 邮箱走"忘记密码"重置。需同时掌控 A 和 B 的 root 邮箱；移出 Org 前也要先恢复 root。

---

## 附录：实测验证结论（账号实例加入 Org 非破坏性）

在测试账号上实际跑通了"B 加入 A"，结论如下，可作为生产前的判断依据。

**结论：当 B 的 Identity Center 是「账号实例（account instance）」时，加入 A 的 Organization 是非破坏性的——实例不重建、用户不丢失。**

**判定方法（最硬的证据是"实例 ID 加入前后不变"）：**

```bash
# 加入前后各跑一次，对比 InstanceArn / IdentityStoreId 是否一致
aws sso-admin list-instances
```
...
| 项目 | 加入 A 前 | 加入 A 后 | 判断 |
| --- | --- | --- | --- |
| B 的实例 `IdentityStoreId` | `d-xxxx`（不变） | `d-xxxx`（同一个） | ✅ 实例未重建 |
| 实例 `OwnerAccountId` | = B 账号ID | = B 账号ID | ✅ 仍归 B，即账号实例 |
| 实例 `Status` | `ACTIVE` | `ACTIVE` | ✅ 正常 |
| `describe-organization` 的 `MasterAccountId` | （B 自己/无 Org） | = A 账号ID | ✅ 已加入 A |

**为什么能确定是账号实例**：B 加入后是**成员账号**，而组织实例只归**管理账号 A**；既然 B 自己还拥有一个 `OwnerAccountId = B` 的实例，它**只能是账号实例**。

**重要区分**：实测中若看到用户列表变化，要分清是
- **你自己增删用户**导致的（看 `CreatedBy` / `CreatedAt`）—— 与加入 Org 无关；还是
- **删除 B 旧 Org 导致实例重建**导致的（实例 `IdentityStoreId` 会变）—— 这才是真风险。

**生产前必做**：用 `aws organizations describe-organization` 确认生产 B 是否为某 Org 的管理账号、`list-instances` 确认实例类型。
- 独立账号 + 账号实例 → 与本测试同款，安全；
- 自己 Org 的管理账号 + 组织实例 → 高风险，先备份、在测试账号复刻演练、并开 AWS Support。
